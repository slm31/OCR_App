...
def identify_arabic_letter_from_bytes(image_bytes: bytes, mime_type: str):
    ...
    try:
        ...
        prompt = (
                    "انظر إلى الصورة بدقة وحدد أي حرف عربي منفصل يظهر فيها. "
                    "مهمتك هي التعرف على شكل الحرف فقط، وليس أي نص أو كلمة. "
                    "جميع الصور تحتوي على حروف عربية مفردة ومنفصلة فقط. "
                    "انتبه جدًا إلى الفرق بين الهمزة (ء) والعين (ع): "
                    "الهمزة هي نقطة صغيرة مستقلة أو شكل نصف دائرة صغير على السطر، بدون أي امتداد أو ذيل. "
                    "أما العين فهي حرف له جسم واضح وانحناء مفتوح، وغالبًا يكون أكبر حجمًا من الهمزة. "
                    "يجب أن تكون إجابتك أحد الأحرف التالية فقط: "
                    "ا، أ، إ، آ، ى، ب، ت، ث، ج، ح، خ، د، ذ، ر، ز، س، ش، ص، ض، ط، ظ، ع، غ، ف، ق، ك، ل، م، ن، هـ، و، ؤ، ي، ئ، ة، ء. "
                    "أجب بالحرف نفسه فقط بدون أي شرح أو رموز أو نص إضافي."
                )
        contents = [
